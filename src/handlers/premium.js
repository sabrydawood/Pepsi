    ...
    if (premiumdb) {
      if (premiumdb.status.enabled) {
        ...
        if (plane === "365") {
          ...
          planeText = "No Plane This is Test Only";
        }
        ...
        if (now - premiumdb.status.time > cooldown) {
          const OwnerEmbed = new EmbedBuilder()
            ...
            .setDescription(
              "Dear sir Your guild Premium features has been Finished if you need to repay it you can use Command `getpremium`"
            )
            .addFields(
              {
                name: "Guild Name",
              ...
              {
                name: "Start At",
                value: new Date(premiumdb.status.start).toLocaleDateString()
                  ? new Date(premiumdb.status.start).toLocaleDateString()
                  : "No Data",
              },
              {
                name: "Ends At",
                value: new Date(premiumdb.status.end).toLocaleDateString()
                  ? new Date(premiumdb.status.end).toLocaleDateString()
                  : "No Data",
              },
          ...
          const HookEmbed = new EmbedBuilder()
            ...
            .setDescription("guild Premium features has been Finished")
            .addFields(
              {
                name: "Guild Name",
