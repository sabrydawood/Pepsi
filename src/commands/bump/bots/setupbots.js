...
module.exports = {
  ...
  async interactionRun(interaction, data) {
    ...
    //setup invite link
      ...
      const invite = interaction.options.getString("link");
...
async function setChannel(settings, channel, lang) {
  if (settings.bump.bots.channel_id === channel.id)
    return "You have already seted this channel try another channel please if you need change it";
...
async function setInvite(settings, invite, lang) {
  //let checker = await containsLink(invite);
  // if(checker){
  settings.bump.bots.invite_link = invite;
  await settings.save();
  return "Successfully configuration settings now invite link set to " + settings.bump.bots.invite_link;
