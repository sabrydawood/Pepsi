    ...
    options: [
      {
        ...
        type: ApplicationCommandOptionType.String,
         required: true,
        choices :[{ name: '1 day test', value: "test" },
...
//{ name: 'Free', value: "trail" },
 { name: 'Cash', value: "cash" },
  ...
  let time = interaction.options.getString("time"), method = interaction.options.getString("method")
    const response = await addPremium(data.userDb, premiumDb,time,method, data.lang);
...
desc =  "sorry you didn't add supported time `Supported times => `"+"\n\`\`\`js\n" +  times +"\n\`\`\`";  
   ...
   if (premium.status.isTested){
    pay = "Free Trail"
desc=  "We Are Sorry But this plane can obtain for one time Only Try another plane or check if there is a giveaway at my support server " + "<:MixFace4:803004709765251083>"; 
 ...
 Math.round(cooldown - (now - premium.status.time)),
      ...
      desc =  "your guild is Already Enabled If you mean add time Please Use `refreshpremium` Command you still have time => \n" + "<a:5415_WumpusHypesquad:803558462927405076> "  + remaining 
...
cash = percentCalculation(PREMIUM.CASH.YEAR, 10)
 ...
 if(userDb.bank < coinsToClear ){
   desc = "sorry but you didn't have enough coins into your bank if you have enough at your balance please deposit it first and try again\n you have Now coins at bank `" + userDb.bank + "` Coins" + "<a:5415_WumpusHypesquad:803558462927405076>"
...
guildName = premium.data.name;
    desc = "Successfully enabled premium featrus Congratulations 🎉 For a " + time + " plane end at " + new Date(now + timedate * 24 *60* 60 * 1000).toLocaleDateString();
...
function splArr (value){
    value.map((a) => {
        return {name:a.name, val:a.value}
        })
    }